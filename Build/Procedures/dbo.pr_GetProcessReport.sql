...
--exec [dbo].[pr_GetProcessReport] 'CathPCI'

CREATE PROCEDURE [dbo].[pr_GetProcessReport] @Registry VARCHAR(100) = NULL AS
BEGIN 
...
--Added on 4/19/2018 for CathPCI
UNION ALL 

SELECT RegistryName = 'CathPCI', Timeframe = MAX(ad.TimeFrameDisplayText), ProcessType = ad.AggregationType, SubmissionsProcessed = COUNT(*), ProcessCompletedtime = MAX(ad.UpdateTime) 
FROM CathPCIDM.dbo.SubmissionSnapshot csm
INNER JOIN CathPCIDM.dbo.AggregationDIM ad ON ad.AggregationKey = csm.AggregationKey
WHERE ad.RowIsCurrent = 1
GROUP BY ad.AggregationType
)
